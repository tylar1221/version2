...
# =============================
def load_labels():
    """Load all labels from Supabase and cache in session state"""
    res = supabase.table("image_damage_labels").select("*").execute()
    return {r["image_path"]: r for r in res.data if r["image_path"]}
...
def get_images_page(folder, page=0, unlabeled_only=True):
    """Get images for a specific page with optional filtering"""
    if not folder:
        return []
    ...
    # Filter out labeled images if unlabeled_only is True
    if unlabeled_only:
        labeled_paths = set(st.session_state.labels.keys())
        images = [
            img for img in images
            if f"{folder}/{img['name']}" not in labeled_paths
        ]
...
def save_label(image_name, description, side, severity):
    """Save label to Supabase and refresh local cache"""
    image_path = f"{st.session_state.selected_folder}/{image_name}"
    ...
    if existing.data:
        ...
        supabase.table("image_damage_labels").insert(payload).execute()

    # Refresh labels cache immediately
    st.session_state.labels = load_labels()
    
    # Also remove this image from current images list if filtering is on
    if st.session_state.filter_unlabeled:
        # Remove the labeled image from current page
        ...
        if st.session_state.current_index >= len(st.session_state.images):
            st.session_state.current_index = max(0, len(st.session_state.images) - 1)
...
with tab1:
    ...
    # Apply filter change
    if filter_state != st.session_state.filter_unlabeled:
        st.session_state.filter_unlabeled = filter_state
        st.session_state.page = 0
        st.session_state.current_index = 0
        st.session_state.images = get_images_page(
            st.session_state.selected_folder,
    ...
    with col1:
        ...
        st.caption(
            f"Page {st.session_state.page + 1} | "
            f"Image {st.session_state.current_index + 1} "
            f"of {len(st.session_state.images)}"
        )

    with col2:
        image_path = (
            f"{st.session_state.selected_folder}/"
            f"{st.session_state.current_name}"
        )
        existing = st.session_state.labels.get(image_path, {})
        ...
        with st.form("label_form"):
            ...
            if save_next_btn and description.strip():
                ...
                # Move to next image
                if st.session_state.images:  # Check if still have images
                    if st.session_state.current_index < len(st.session_state.images) - 1:
                        st.session_state.current_index += 1
                    else:
                        # Go to next page
