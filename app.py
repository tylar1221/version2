...
defaults = {
    ...
    "images": [],
    "labels": {},
    "labels_refresh_counter": 0,  # NEW: Force refresh when incremented
    "current_index": 0,
    "filter_unlabeled": True,
    "current_name": None,
    "current_image": None,
...
def load_labels(force_refresh=False):
    """Load labels from Supabase with optional force refresh"""
    if force_refresh or not st.session_state.get('labels_last_refresh'):
        # Clear any cache and fetch fresh
        res = supabase.table("image_damage_labels").select("*").execute()
        st.session_state['labels_last_refresh'] = pd.Timestamp.now()
        return {r["image_name"]: r for r in res.data}
...
def get_images_page(folder, page=0, unlabeled_only=True):
    """FIXED: Always use fresh labels when filtering"""
    if not folder:
        return []
    ...
    # FORCE refresh labels if filtering unlabeled
    if unlabeled_only:
        # Get fresh labels from database
        fresh_labels = load_labels(force_refresh=True)
        st.session_state.labels = fresh_labels  # Update session state
...
def save_label(image_name, description, side, severity):
    """FIXED: Clear image cache after saving"""
    image_path = f"{st.session_state.selected_folder}/{image_name}"
    ...
    # Force refresh labels AND increment counter
    st.session_state.labels = load_labels(force_refresh=True)
    st.session_state.labels_refresh_counter += 1  # Force UI refresh
    
    # Clear image cache for this folder
    if 'image_cache' in st.session_state:
        cache_key = f"{st.session_state.selected_folder}_page_{st.session_state.page}"
        if cache_key in st.session_state.image_cache:
            del st.session_state.image_cache[cache_key]
...
# =============================
# TAB 1: LABELING - FIXED VERSION
# =============================
with tab1:
    folders = get_folders()
    selected = st.selectbox("📁 Select Batch Folder", folders)
    
    # Check if we need to refresh due to label changes
    ...
    if selected != st.session_state.selected_folder or refresh_needed:
        ...
        st.session_state.page = 0
        st.session_state.current_index = 0
        # Force get fresh images with updated labels
        st.session_state.images = get_images_page(
            selected, 0, st.session_state.filter_unlabeled
        )
        load_current_image()
    ...
    with col_left:
        # Use a callback to force refresh when checkbox changes
        def update_filter():
            st.session_state.page = 0
            st.session_state.current_index = 0
            # Force fresh load with current filter
            st.session_state.images = get_images_page(
                st.session_state.selected_folder,
                0,
                st.session_state.filter_unlabeled
        ...
        st.checkbox(
            ...
            help="⚠️ Note: This uses LIVE data from Supabase. Images labeled just now will disappear immediately."
        )
    ...
    if not st.session_state.images:
        ...
        if st.session_state.selected_folder:
            ...
            st.metric(f"📊 Batch {st.session_state.selected_folder} Progress", 
    ...
    with col1:
        ...
        if st.session_state.current_name in st.session_state.labels:
            ...
            label_data = st.session_state.labels[st.session_state.current_name]
            st.write(f"**Side:** {label_data.get('side', 'N/A')}")
            st.write(f"**Severity:** {label_data.get('severity', 'N/A')}")
            st.write(f"**Description:** {label_data.get('description', 'N/A')}")
    ...
    with col2:
        ...
        with st.form("label_form"):
            ...
            if save_next_btn and description.strip():
                ...
                st.success(f"✅ Label saved for {st.session_state.current_name}")
...
with tab2:
    ...
    if 'image_path' in df.columns:
        df['batch'] = df['image_path'].apply(lambda x: x.split('/')[0] if '/' in x else 'unknown')
        batch_stats = df.groupby('batch').size().reset_index(name='count')
        ...
        for _, row in batch_stats.iterrows():
            # Count total images in that batch
            try:
                batch_images = supabase.storage.from_(BUCKET_NAME).list(row['batch'])
                total_in_batch = len([f for f in batch_images 
                                    if f["name"].lower().endswith((".jpg", ".jpeg", ".png"))])
                progress = row['count'] / total_in_batch if total_in_batch > 0 else 0
                st.progress(progress, text=f"Batch {row['batch']}: {row['count']}/{total_in_batch} ({progress*100:.0f}%)")
            except:
                st.text(f"Batch {row['batch']}: {row['count']} labels")
...
with st.expander("🔧 Debug Info (Optional)"):
    ...
    st.json({k: str(v)[:100] + "..." if isinstance(v, dict) and len(str(v)) > 100 else v 
             for k, v in st.session_state.items()})
    
    st.write("### Database Status")
    st.write(f"Labels in session state: {len(st.session_state.labels)}")
    st.write(f"Images in current page: {len(st.session_state.images)}")
